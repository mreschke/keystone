...
class Keystone implements KeystoneInterface
{
	...
	public function __construct(RedisDatabase $redis, Meta $meta)
	{
		$this->redis  = $redis->connection('keystone');
		$this->prefix = Config::get('database.redis.keystone.prefix', 'keystone:');
		$this->rootNs = Config::get('database.redis.keystone.root_namespace', 'mreschke/foundation');
		$this->metaNs = Config::get('database.redis.keystone.metadata_namespace', 'mreschke/keystone');
		$this->path   = Config::get('database.redis.keystone.path');
		$this->maxRedisSize = Config::get('database.redis.keystone.max_redis_size', 4096);
			...
			if ($type == 'hash') {
				...
				if ($this->isAssoc($value)) {
					...
					if (is_array($index)) {
						...
						$return = array();
						foreach ($index as $item) {
							if (property_exists($value, $item)) $return[$item] = $value->$item;
						}
						return $return;
					} else {
						// Return single value
	...
	public function put($key, $value, $serialize = false)
	{
		$this->transaction($key, function($key) use($value, $serialize) {
			...
			if ($serialize || is_object($value)) $value = serialize($value);
		...
		if ($this->exists($key)) {
			...
			$this->transaction($key, function($key) use($value) {
				...
				if ($type == 'list') {
					...
					if (is_string($value)) $value = [$value]; // convert to array
					foreach ($value as $item) {
						$this->redis->rpush($key, $item);
					}
	...
	public function forget($key, $index = null)
	{
		$this->transaction($key, function($key) use($index) {
			if (isset($index)) {
				...
				if (is_string($index)) $index = [$index]; // convert to array
				$type = $this->redis->type($key);
				if ($type == 'list') {
					// Remove from list
					foreach ($index as $item) {
	...
	private function transaction($key, $transaction)
	{
		...
		if (is_null($this->ns)) $this->ns = $this->rootNs;

		// Prepend ns to key
		if (!str_contains($key, '::')) $key = $this->ns.'::'.$key;

		// Prepend redis prefix to key
		if (substr($key, 0, strlen($this->prefix)) != $this->prefix) $key = $this->prefix.$key;
